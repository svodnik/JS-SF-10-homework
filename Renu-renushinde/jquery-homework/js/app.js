...
//const $sumbitBtn=  $("#addQuestion");   // submit button
/*const $questionText =$("#question");    // input from user
const $surveyList = $('#surveyList');   // parent Ol
const $questions = $('.form-group padout');  // children li
*/

$('#addQuestion').on('click', function(event){
   event.preventDefault();
   //console.log("clicked!");
   var $askQuestion= $('#question').val();   // grabbed the value from the input field.
   //console.log(question);

   // create a new Li and add it to Ol
   $("ol").append('<li class=form-group padout><label>' + $askQuestion + '</label><input type="text" class="form-control"><button>'+ "Remove question"+'</button></input></li>');
   $('#question').val('');  // to clear the value from input

});

$('ol').on('click','button', function(event){  //event delegation
   event.preventDefault();
   $(this).parent().remove();
});