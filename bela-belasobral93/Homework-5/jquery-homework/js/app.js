...
$('#addQuestion').on('click', function(event){
   ...
   if ($newQuestion === ""){
      ...
      $('ol').append('<li class=form-group padout><label>' + $newQuestion + '</label><input type="text" class="form-control"><button>'+ "Remove question"+'</button></input></li>');
   }
   $('#question').val('');
...
/*Create an event listener for all survey questions that adds the bg-warning
   class value to the element in response to the mouseenter event, and
   removes it in response to the mouseleave event. When the mouseenter event
   happens, the bg-warning class should also be removed from all sibling
   elements. */
   ...
   $('ol').on('mouseenter mouseleave','label',function(event){
      if(event.type ==='mouseenter'){
         ...
         $(this).siblings().addClass('bg-warning'); //chaining
