...
const $newQuestionCreate = $('#addQuestion');
const $allQuestions = $('#surveyList');

function addQuestion(content) {
   const $contentLi = $('<li>').addClass('form-group padout');
   const $contentLabel = $('<label>').html(content);
   // 5. Below is an example of multiple chaining
   const $contentInput = $('<input>').attr('type', 'text').addClass('form-control');
   const $contentButton = $('<button>').html('Remove question');
   // 5. Below is an example of multiple chaining
   const $newBlock = $contentLi.append($contentLabel).append($contentInput).append($contentButton);
   // 5. Below is an example of multiple chaining
   $allQuestions.append($contentLi.wrap($newBlock));
}

$newQuestionCreate.on('click', function(event){
...
// 5. Below is an example of event delegation
$('body').on('click', 'button', function(event){
   event.preventDefault();
   $(this).parent().remove();
}); 

$allQuestions.on('mouseenter mouseleave', 'li', function(event) {
