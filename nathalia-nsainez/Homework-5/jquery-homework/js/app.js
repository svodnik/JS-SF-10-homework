...
const form=document.querySelector('#finalSurvey');
const style=document.querySelector('voffset bg-info');
...
addQuestion.addEventListener('click', function(event){
    ...
    newQuestionItem.className = style;
    let questionText = document.createTextNode(newQuestion);
    newQuestionItem.appendChild(questionText);
    list.appendChild(newQuestionItem);
...
$(document).ready(function(){
    ...
    const $button = $('#addQuestion');


$button.on('click', function(event) {
    event.preventDefault();
    const newThing = $questionInput.val();
    $('ol').append('<li class=form-group padout><label>' + newThing + '</label><input type="text" class="form-control"><button>'+ "Remove question"+'</button></input></li>');
...
$('ol').on('mouseenter mouseleave', 'label', function(event){
    if(event.type ==='mouseenter'){
        $(this).addClass('bg-warning');
        $(this).siblings().addClass('bg-warning');   //CHAINING 2 METHODS IN SINGLE SELECTOR
    }
