...
var changing = document.querySelector('#roll-dice');
changing.addEventListener('click', function(){
  random1 = random[Math.floor(Math.random() * random.length)];
  random2 = random[Math.floor(Math.random() * random.length)];
  let firstDie = ('dice dice-'+random1);
  let secondDie = ('dice dice-'+random2);
